...
// 共通の型定義
type FieldValueType = string | number | string[];

// stringは検証できるが他の型を考慮していないので型制約をつける
type AvailableFiledType = {[key: string]: FieldValueType};

// IDEで補完が効くように、Genericsで検証対象フィールドを指定させてそれに基づいた型制約をつける
type FieldConstraintSchema<T> = Record<keyof T, Constraint<FieldValueType | null>>;
...
class ConstraintValidators<T extends AvailableFiledType> {
  ...
  constructor(schema: FieldConstraintSchema<T>, setValidationError: SetValidationError<T>) {
    ...
    for (const key of Object.keys(schema)) {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any -- any型の修正の影響範囲が大きく、修正コストが高いためany型を使用。
